  ...
  <div class="panel">
    ...
    <div class="panel-body">
      ...
      <el-table :data="table_data" v-loading="load_data" element-loading-text="拼命加载中"
        ...
        <el-table-column label="操作" width="170">
          <template scope="props">
            <router-link :to="{name: 'dirUpdate', params: {dirId: props.row.tableId}}" tag="span">
              <el-button type="info" size="small" icon="edit">修改</el-button>
            </router-link>
...
export default {
  ...
  methods: {
    ...
    get_dir_data() {
      var vm = this;
      var curr = this.dirCurrPage;
      var total = this.dirTotal;
      this.load_dir = true;
      this.$http({
        url: "/api/tt",
        method: "get"
        ...
        .then(function(res) {
          if (res.data.result == "success") {
            vm.$message.success('请求成功' + res.data.result);
            console.log(res);
            // vm.dir_data = res.data.message;
            curr = 1;
            total = 120;
          } else {
            vm.$message.error("获取失败，" + res.data.reason);
          }
        })
        .catch(function(err) {
          if(err.response)
            vm.$message.error('网络连接错误' + err.response.status);
          console.log(err, "错误");
        });
      this.load_dir = false;
    },
