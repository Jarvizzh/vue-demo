        ...
        <el-col :span="8">
          <el-form ref="form" :model="form" :rules="rules" label-width="150px">
            ...
            <el-form-item label="权限分配:">
               <span>读权限：</span>
               <el-select v-model="form.rapps" multiple filterable placeholder="请选择应用组" style="width: 380px;">
                  <el-option
                    v-for="item in appOptions"
                    :key="item.appId"
                    :label="item.appName"
...
export default {
  ...
  methods: {
    get_app_list() {
      var vm = this;
      this.$http({
        url: "/api/app/list", //api 代理到json文件地址，后面的后缀是文件中的对象或者是数组
        method: "get" //请求方式
        //这里可以添加axios文档中的各种配置
      })
        .then(function(res) {
          if (res.data.code == 200) {
            vm.appOptions = res.data.data;
          }
    ...
    //提交
    on_submit_form() {
      var formData = this.$qs.stringify(this.form);
